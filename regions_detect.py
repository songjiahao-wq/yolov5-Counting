...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        x11,y11,x12,y12,x13,y13,x14,y14=656,239,1389,266,1546,741,236,723
        ...
        # mask for certain region2
        # ***********************************
        # 1,2,3,4 分别对应左上，右上，右下，左下四个点
        x211, y211, x212, y212, x213, y213, x214, y214 = 126,23,147,22,151,73,124,70
        h2l1 = round(y211 / ima_h, 2)  # 监测区域高度距离图片顶部比例
        w2l1 = round(x211 / ima_w, 2)  # 监测区域高度距离图片左部比例
        h2l2 = round(y212 / ima_h, 2)  # 监测区域高度距离图片顶部比例
        w2l2 = round(x212 / ima_w, 2)  # 监测区域高度距离图片左部比例
        h2l3 = round(y213 / ima_h, 2)  # 监测区域高度距离图片顶部比例
        w2l3 = round(x213 / ima_w, 2)  # 监测区域高度距离图片左部比例
        h2l4 = round(y214 / ima_h, 2)  # 监测区域高度距离图片顶部比例
        w2l4 = round(x214 / ima_w, 2)  # 监测区域高度距离图片左部比例
        ...
        if webcam:
            ...
            pts2 = np.array([[int(im.shape[2] * w2l1), int(im.shape[1] * h2l1)],  # pts1
                            [int(im.shape[2] * w2l2), int(im.shape[1] * h2l2)],  # pts2
                            [int(im.shape[2] * w2l3), int(im.shape[1] * h2l3)],  # pts3
                            [int(im.shape[2] * w2l4), int(im.shape[1] * h2l4)]], np.int32)
            mask = cv2.fillPoly(mask, [pts,pts2], (255, 255, 255))
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if webcam:  # batch_size >= 1
                ...
                zeros = np.zeros((im0.shape), dtype=np.uint8)
                mask = cv2.fillPoly(zeros, [pts], color=(0, 165, 255))
                im0 = cv2.addWeighted(im0, 1, mask, 0.2, 0)#合成

                cv2.polylines(im0, [pts], True, (255, 255, 0), 3)
            # **************划线*************************
            # ima_w, ima_h = 480, 343
            # # 左坐标到右坐标
            # x1, y1, x2, y2 = 168, 197, 419, 197
            # line = [(x1, y1),
            #         (x2, y2)]  # 图像信息的三个通道
            ...
            # *********************************
            # cv2.putText(im0, f"up:50 cars", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2,
            #             (0, 0, 255), 2)
            # *********************************
            # Stream results
...
def parse_opt():
    ...
    parser.add_argument('--classes',default=[0,9], nargs='+', type=int, help='filter by class: --classes 0, or --classes 0 2 3')
